...
@pytest.mark.asyncio
async def test_save_qaa_title_new_collection(
    qaa_service, mock_author, mock_bot, mock_collection_model
):
    ...
    with patch(
        "services.qaa_service.back_to_message_menu", return_value="keyboard_mock"
    ):
    ...
    mock_bot.send_message.assert_awaited_once_with(
        mock_author.id, "با موفقیت ذخیره شد", "keyboard_mock"
    )


@pytest.mark.asyncio
async def test_save_qaa_title_existing_collection(
    qaa_service, mock_author, mock_bot, mock_collection_model
):
    ...
    mock_bot.send_message.assert_awaited_once_with(
        mock_author.id, "شما آن را دارید\nلطفا عنوانی جدید وارد کنید", "keyboard_mock"
    )
...
async def test_save_qaa_state_1_sets_temp_data(qaa_service, mock_author, mock_bot):
    ...
    mock_bot.send_message.assert_awaited_once_with(
        mock_author.id, "متن پرسش رو وارد کن"
    )
...
@pytest.mark.asyncio
async def test_handel_qaa_answers_completes(
    qaa_service, mock_author, mock_bot, mock_user_model
):
