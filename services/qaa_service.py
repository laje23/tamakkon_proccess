...
class QAAService():
    ...
    async def save_qaa_state_1(self , author , text):
        ...
        if x:=self.model_collection.collection_exists(text):
            await self.bot.send_message(author.id , 'شما آن را دارید\nاگر مایل به اضافه کردن سوال هستید متن ان را ارسال کنید' , back_menu())
    ...
    async def save_qaa_state_6(self, author , text ):
        ...
        self.model_question.add_question(collection_id , question_text , option_1 , option_2 , option_3 , correct_option)
        
        author.del_state()
        await self.bot.send_message(author.id , 'پرسش و پاسخ با موفقیت ثبت شد' , back_menu()) 